...
void test_success() {
    ...
    if (used_size != 16){
      flag = 0;
      push_info_msg(_("You didn't allocate the right amount of memory."));
    ...
    if(stats.malloc.called > 1){
        flag = 0;
        push_info_msg(_("Why did you call malloc more then once?"));
...
void test_fail_first() {
    ...
    if(stats.malloc.called > 2){
        flag = 0;
        push_info_msg(_("Why did you call malloc more then once per loop?"));
        set_tag("not_malloc_once");
    }

    CU_ASSERT_EQUAL(stats.sleep.called,1)
    if(stats.sleep.called > 1) {
        push_info_msg(_("Why did you call sleep more then once per loop?"));
        flag = 0;
    }
