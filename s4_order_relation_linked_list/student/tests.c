...
void test_empty_list(){
    ...
    if(delta != sizeof(node_t)){
	flag_insert = 0;
	set_tag("wrong_alloc_size");
	push_info_msg(_("You don't allocate the right amount of memory"));
    }
    CU_ASSERT_EQUAL(ret,0);
    if(ret){
	flag_insert = 0;
	set_tag("error_normal_case");
	push_info_msg(_("You don't return the good value"));
    }
	...
	push_info_msg("You must allocate dynamic memory on the heap rather then static variables on the stack");
    }
    CU_ASSERT_EQUAL(stats.malloc.called,1);
    if(stats.malloc.called != 1){
	flag_insert = 0;
	set_tag("not_malloc_once");
	push_info_msg(_("You only have to call malloc once to insert a node"));
    }
    if(*h){
	...
	if((*h)->val != 'c'){
		...
		push_info_msg(_("The value of your created node is not correct."));
	}
        CU_ASSERT_PTR_NULL((*h)->next);
	if((*h)->next != NULL){
		flag_insert = 0;
		set_tag("bad_next_pointer");
		push_info_msg(_("The pointer to the next node is supposed to be NULL."));
	}
    }
    else{
	flag_insert = 0;
...
void test_empty_list_fail(){
    ...
    if(delta){
	flag_insert = 0;
	set_tag("modif_memory_malloc_fail");
	push_info_msg(_("You shouldn't allocate memory if malloc fails"));
    }
    ...
    if(*h){
	flag_insert = 0;
	push_info_msg(_("The head sould be NULL"));
    }
    if (stats.malloc.called !=1){
	flag_insert = 0;
	set_tag("not_malloc_once");
	push_info_msg(_("Malloc was not called exactly once"));
    }
}
...
void test_insert_first_cresc(){
	...
	flag_insert = 0;
	set_tag("wrong_alloc_size");
	push_info_msg(_("You don't allocate the good amount of memory"));
    }
	...
	flag_insert = 0;
	set_tag("not_allocated");
	push_info_msg("You must allocate dynamic memory on the heap rather than static variables on the stack");
    }
...
void test_insert_first_fails(){
    ...
    int ret = 1;
    node_t* l = (node_t*) malloc(sizeof(node_t));
    if(!l) { flag_insert = 0; CU_FAIL(_("no mem")); return;}
    l->val = 'c';
    l->next = NULL;
    node_t** h = &l;
	...
	if((*h)->val != 'c'){
	    ...
	    set_tag("bad_next_pointer");
	    flag_insert = 0;
            CU_FAIL(_("You change the list if malloc fails. The pointer to the next node is supposed to be NULL."));
            push_info_msg(_("You change the list if malloc fails. The pointer to the next node is supposed to be NULL."));
        }
    }
    else{
	flag_insert = 0;
	set_tag("not_expected_list");
        CU_FAIL(_("You shouldn't change the head"));
        push_info_msg(_("You shouldn't change the head"));
    }
...
void test2_insert_middle_cresc(){
    ...
    int ret = 1;
    node_t* l1 = (node_t*) malloc(sizeof(node_t));
    if(!l1) { flag_insert = 0; CU_FAIL(_("no mem")); return;}
    node_t* l2 = (node_t*) malloc(sizeof(node_t));
    if(!l2) { flag_insert = 0; CU_FAIL(_("no mem")); free(l1); return;}
    node_t* l3 = (node_t*) malloc(sizeof(node_t));
    if(!l3) { flag_insert = 0; CU_FAIL(_("no mem")); free(l1); free(l2); return;}
    node_t* l4 = (node_t*) malloc(sizeof(node_t));
    if(!l4) { flag_insert = 0; CU_FAIL(_("no mem")); free(l1); free(l2); free(l3); return;}
    node_t* l5 = (node_t*) malloc(sizeof(node_t));
    if(!l5) { flag_insert = 0; CU_FAIL(_("no mem")); free(l1); free(l2); free(l3); free(l4); return;}
    l5->val = 'f';
    l5->next = NULL;
    l4->val = 'd';
    l4->next = l5;
...
void test1_insert_middle_fails(){
    ...
    if(delta){
	flag_insert = 0;
	set_tag("modif_memory_malloc_failed");
	push_info_msg(_("You shouldn't allocate memory when malloc fails"));
    }
    CU_ASSERT_EQUAL(ret,-1);
    if(!ret){
	flag_insert = 0;
	...
	if((*h)->val != 'a'){
	    ...
	    if((*h)->next->val != 'c'|| (*h)->next->next){
		flag_insert = 0;
		set_tag("not_expected_list");
		push_info_msg(_("You must not change the list."));
	    }
        }
        else{
	   set_tag("bad_next_pointer");
	   flag_insert = 0;
	   push_info_msg(_("You must not change the list. The pointer to the next node is supposed to be NULL."));
	}
    }
    else{
	flag_insert = 0;
...
void test_reinsert_first(){
    ...
    if(delta){
	flag_insert = 0;
	push_info_msg(_("You shouldn't allocate memory for a reinsertion"));
    }
    CU_ASSERT_EQUAL(ret,0);
    if(ret){
	flag_insert = 0;
    ...
    if(ret){
	flag_insert = 0;
	push_info_msg(_("You must not reinsert elements"));
    }
    switch (ret) {
        case -1:
	    flag_insert = 0;
...
void test_reinsert_middle(){
	...
	flag_insert = 0;
	push_info_msg(_("You don't have to call malloc"));
    }
